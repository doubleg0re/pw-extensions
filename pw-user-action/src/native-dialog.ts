...
import type { UserActionResponse } from './state.js';
import { subscribeMonitor, type MonitorState, type MonitorSubscription } from './monitor-ws-client.js';
...
export function resolveNativeDialogBinary(): string {
...
export function canUseNativeDialog(): boolean {
...
export async function showNativeDialog(opts: NativeDialogOptions): Promise<UserActionResponse> {
  ...
  const eventWaiters: Array<(e: RendererEvent) => void> = [];
  let stdoutBuffer = '';
  ...
  child.stdout!.on('data', chunk => {
    ...
    while ((idx = stdoutBuffer.indexOf('\n')) !== -1) {
      ...
      if (!line) continue;
      try {
        const ev = JSON.parse(line) as RendererEvent;
  ...
  function nextEvent(timeoutMs = 30000): Promise<RendererEvent> {
    return new Promise((resolve, reject) => {
      ...
      const resolver = (e: RendererEvent) => {
        clearTimeout(timer);
        resolve(e);
      };
  ...
  try {
    ...
    let subscription: MonitorSubscription | null = null;
    let lastSentVisible: boolean | null = null;
    ...
    let ownerTabId: number | null = opts.tabId && opts.tabId > 0 ? opts.tabId : null;
    ...
    const computeVisible = (state: MonitorState): boolean => {
      ...
      if (ownerTabId == null) {
        ...
        if (typeof active === 'number' && active > 0) ownerTabId = active;
        return true;
      }
    ...
    const sendVisibility = (visible: boolean) => {
      if (visible === lastSentVisible) return;
      try {
