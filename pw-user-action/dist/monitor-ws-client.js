...
async function loadPwMonitorClient() {
    if (cachedClient)
        return cachedClient;
    const candidates = [
    ...
    if (!entry) {
        throw new Error('pw-monitor client module not found. Ensure pw-monitor is installed in the toybox (check extension.dependencies).');
    }
